 ...
 * 한진택배정보체계
    ...
    ### 2021.10.01 ~ 2023.04.16
* 프로젝트 종료
#### 느낀점: 내 인생의 첫 개발 프로젝트가 끝났다. 설계단계부터 구현단계까지의 과정을 통해 많은 것을 경험하였다. 게다가 난 개발자로서의 첫 프로젝트였기에 모르는 점도 많았지만 우리 팀원들과 다른 개발자들의 도움을 많이 받았다. 또한 새로운 기술이나 지식을 습득하기 위해 집에서 개인적으로 넥사크로, 잡패스, 전자정부프레임워크 등 다양한 기술들을 공부하여 프로젝트를 진행하는 데에 있어서 나에게 주어진 업무를 성공적으로 수행하려고 노력하였고 그 결과 프로젝트가 끝난 나는 시작할때보다 훨씬 성장하게 되었다. 그리고 국방부 청사에서 근무하며 프로젝트를 진행하였는데 내가 살면서 국방부에 들어올 일이 있을까 싶기도하고 또한 내가 개발하는 기능들과 화면이 실제 군인들이 사용할 것이라고 생각을 하니 자부심이 들면서 동시에 최선을 다해야겠다고 다짐하게 되었다. 마지막으로 이번 프로젝트를 통해 가장 크게 느낀점은 내가 열심히 노력한다면 개발자로서의 삶이 나쁘지 않을 것 같다라는 희망과 자신감이 생겼다는 것이다. 이번 프로젝트를 통해 얻은 지식과 경험을 잊지 않고 어떤 프로젝트를 하던 수많은 새로운 기술들을 접해도 나는 성공적으로 해낼 것이다. 

   ### 2023.01.16 ~ 2023.01.27
  * TRR(Test Readiness Review-시험 준비 검토) 2차 평가 실시 완료 및 산출물 작성
#### 느낀점: TRR 2차 평가가 성공적으로 마무리가 되었다. 걱정과는 달리 모든 부분에서 통과가 되었으며 큰 특이사항없는 개선사항만 나와 이를 반영하는 작업을 하였다. 그리고 오랜만에 산출물을 작성하였다. 산출물은 사용자인터페이스설계서, 메뉴구조도, 사용자지침서이다. 예전에 이 3가지 산출물을 이미 작성하였으나. 시간이 지나면서 개선사항 등등 많은 요소들로 인해 개발물들이 많이 수정되었다. 그렇기 때문에 수정된 개발물들을 산출물에 반영하는 작업이 필요했다. 특히 가장 수정사항이 많았던 산출물은 사용자지침서이다. 사용자지침서에는 비즈니스 흐름도, 화면UI, 기능, 유의사항 등등 많은 부분들을 작성해야하는데 기존에 작성된 산출물들을 보니 거의 새로 작성을 해야했다. 다른 부분들은 무난하게 작성을 하였으나 특히 비즈니스 흐름도 같은 경우에는 액터가 존재해서 비즈니스 흐름에 따라 메뉴의 순서, 연관된 데이터 정보, 팝업화면 등등 많은 사항들을 흐름도에 녹여내야 했기 때문에 비즈니스 흐름도 작성하는 데에 시간이 오래 걸렸다. 예를 들어 군운전면허시험 같은 경우에 처음 교육부대 면허시험 관련자가 면허시험을 등록하고 신청자가 등록된 면허시험을 신청하면 교육부대 담당자가 면허시험을 치를 응시자를 선정한뒤 시험결과 입력 및 면허증 신청 등 일련의 흐름을 PPT로 과정을 녹여내야 했기 때문에 작성하는 데에 시간이 오래걸렸다. 그렇지만 지금의 산출물을 정확하게 작성을 해야 추후 다른 개발자분이 유지보수를 할 때에 도움이 많이 되기 때문에 구체적으로 작성을 하였다.

  ### 2023.01.02 ~ 2023.01.13
  * TRR(Test Readiness Review-시험 준비 검토) 2차 평가 실시 및 보완
 #### 느낀점: TRR 2차 평가가 시작되었다. 내가 맡았던 분량인 운전자력, 군전세객차에 대해서 TRR 1차에 결함이 났던 부분을 중점적으로 보고 그 이외에 요구사항들이 반영이 되었는지에 대해 보기로 하였다. 다행히 결함난 건들은 해결을 확인하였고 대부분의 요구사항들이 반영되어서 평가위원들이 만족해하셨다. 그런데 문제가 생겼다. 과장님의 파트인 공수근무에서 심각한 결함들이 많이 발생하였다. 과장님이 결함들을 해결을 하려고 하였으나 결함사항이 너무 많기도 하였고 게다가 엎친 데 덮친 격으로 과장님의 아버님이 몸 상태가 많이 안좋아져서 중환자실에 입원을 하게 되었다. 그래서인지 과장님께서 집중을 잘 못하시고 결함처리에 진척이 나가지 않았다. 다른 팀원들은 추가로 나온 결함사항을 해결하느라 바빴지만 나는 결함사항은 없고 요구사항만 몇 개 나왔기 때문에 요구사항을 처리한 뒤 과장님의 공수근무를 도와주기로 하였다. 새로운 업무의 영역이라 DB 구조라든지 업무 프로세스를 전혀 모르고 있어 일 하면서 과장님이나 차장님께 계속 물어가면서 업무를 진행하였다. 나중에는 다른 팀원들이 달라붙어서 공수근무 결함처리를 도와주었고 아무래도 업무 자체를 처음하다보니 진척이 잘 되지 않아 다들 본인 업무가 아님에도 불구하고 주말에 나와서 개발을 하기도 하였다. 다음주 화요일까지 결함처리를 마쳐서 평가위원님의 최종 평가를 하게 되는데 이때 통과를 하지 못하면 프로젝트 전체 테스트 실패가 되어버려 TRR 3차평가가 가게 될 수도 있다. 그래서 다른 프로젝트 인원들에게 피해를 끼치지 않기 위해 개발을 열심히 하였다. 다음에 2차 최종 평가때 제발 결함없이 테스트가 통과되었으면 좋겠다.
